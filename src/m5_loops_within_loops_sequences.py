...
def run_test_integers():
    ...
    expected = [3, 1, 4, 10, 10, 1, 3, 4, 1, 4, 30, -4, 9]
    answer = integers([(3, 1, 4),
                       (10, 'hi', 10),
                       [1, 2.5, 3, 4],
                       'hello',
...
def integers(sequence_of_sequences):
    ...
    #    TIME ESTIMATE:  10 minutes.
    # -------------------------------------------------------------------------
    i = len(sequence_of_sequences)
    list = []
    for k in range(i):
        z = len(sequence_of_sequences[k])
        for j in range(z):
...
def run_test_big_letters():
    ...
    expected = 'OWTSSSOOPSAPSBOSCOPDOO'
    answer = big_letters([(3, 1, 4),  # not a string
                          'Ok What is ThiSSS?',  # OTSSS
                          (10, 'Ok what is ThiSSS?', 10),  # not a string
                          [],  # not a string
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(i):
        z = len(sequence_of_sequences[k])
        test = type(sequence_of_sequences[k])
        if test == str:
            for j in range(z):
                upper = sequence_of_sequences[k][j].isupper()
                if upper == True:
                    list = list + sequence_of_sequences[k][j]
    return list
